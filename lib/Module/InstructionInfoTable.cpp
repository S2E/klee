...
static std::string getFullPath(llvm::StringRef Directory, llvm::StringRef FileName) {
...
public:
    ...
    std::string &getInternedString(const std::string &s) {
        auto found = std::find_if(internedStrings.begin(), internedStrings.end(),
                                  [&s](const std::unique_ptr<std::string> &item) { return *item.get() == s; });
    ...
    std::unique_ptr<FunctionInfo> getFunctionInfo(const llvm::Function &Func) {
        ...
        return std::unique_ptr<FunctionInfo>(new FunctionInfo(0, getInternedString(""), 0, asmLine));
    }
    ...
    std::unique_ptr<InstructionInfo> getInstructionInfo(const llvm::Instruction &Inst, const FunctionInfo *f) {
        ...
        if (dl.get() != nullptr) {
            ...
            // narrow it down
            if (line == 0) {
                if (auto LexicalBlock = llvm::dyn_cast<llvm::DILexicalBlock>(dl.getScope())) {
                    line = LexicalBlock->getLine();
                    column = LexicalBlock->getColumn();
                }
            }
            return std::unique_ptr<InstructionInfo>(
                new InstructionInfo(0, getInternedString(full_path), line, column, asmLine));
        }

        if (f != nullptr)
            // If nothing found, use the surrounding function
            return std::unique_ptr<InstructionInfo>(new InstructionInfo(0, f->file, f->line, 0, asmLine));
        // If nothing found, use the surrounding function
        return std::unique_ptr<InstructionInfo>(new InstructionInfo(0, getInternedString(""), 0, 0, asmLine));
    }
};
...
InstructionInfoTable::InstructionInfoTable(const llvm::Module &m) {
    ...
    for (const auto &Func : m) {
        ...
        for (auto it = llvm::inst_begin(Func), ie = llvm::inst_end(Func); it != ie; ++it) {
            auto instr = &*it;
