...
static void GetAllUndefinedSymbols(Module *M, std::set<std::string> &UndefinedSymbols) {
    ...
    KLEE_DEBUG_WITH_TYPE("klee_linker",
                         dbgs() << "*** Computing undefined symbols for " << M->getModuleIdentifier() << " ***\n");
    ...
    for (auto const &Function : *M) {
        if (Function.hasName()) {
            if (Function.isDeclaration())
                ...
                assert(!Function.hasDLLImportStorageClass() && "Found dllimported non-external symbol!");
    ...
    for (std::set<std::string>::iterator I = UndefinedSymbols.begin(); I != UndefinedSymbols.end(); ++I) {
...
std::unique_ptr<llvm::Module> klee::linkModules(std::vector<std::unique_ptr<llvm::Module>> &modules,
    ...
    for (auto &module : modules) {
        if (!module || !module->getNamedValue(entryFunction))
            continue;
        if (composite) {
            errorMsg = "Function " + entryFunction.str() + " defined in different modules (" +
                       module->getModuleIdentifier() + " already defined in: " + composite->getModuleIdentifier() + ")";
            return nullptr;
        }
    ...
    // fail if not found
    if (!composite) {
        errorMsg = "Entry function '" + entryFunction.str() + "' not found in module.";
        return nullptr;
    }
    ...
    while (true) {
        ...
        for (auto &module : modules) {
            ...
            for (auto symbol : undefinedSymbols) {
                GlobalValue *GV = dyn_cast_or_null<GlobalValue>(module->getNamedValue(symbol));
                if (!GV || GV->isDeclaration())
                    continue;

                // Found symbol, therefore merge in module
                KLEE_DEBUG_WITH_TYPE("klee_linker", dbgs() << "Found " << GV->getName() << " in "
...
Function *klee::getDirectCallTarget(CallSite cs, bool moduleIsFullyLinked) {
    ...
    assert((!viaConstantExpr) && "FIXME: Unresolved direct target for a constant expression");
    return NULL;
}
...
bool klee::loadFile(const std::string &fileName, LLVMContext &context,
                    std::vector<std::unique_ptr<llvm::Module>> &modules, std::string &errorMsg) {
    ...
    if (ec) {
        klee_error("Loading file %s failed: %s", fileName.c_str(), ec.message().c_str());
    }
    ...
    if (magic == sys::fs::file_magic::bitcode) {
        ...
        if (!module) {
            klee_error("Loading file %s failed: %s", fileName.c_str(), Err.getMessage().str().c_str());
        }
...
#endif
#if LLVM_VERSION_CODE >= LLVM_VERSION(3, 9)
        Expected<std::unique_ptr<object::Binary>> archOwner = object::createBinary(Buffer, &context);
        if (!archOwner)
            ec = errorToErrorCode(archOwner.takeError());
        llvm::object::Binary *arch = archOwner.get().get();
#else
        ErrorOr<std::unique_ptr<object::Binary>> archOwner = object::createBinary(Buffer, &context);
...
// Load all bitcode files into memory
            ...
            for (auto AI = archive->child_begin(Err), AE = archive->child_end(); AI != AE; ++AI)
#else
            for (auto AI = archive->child_begin(), AE = archive->child_end(); AI != AE; ++AI)
#endif
            {
...
#if LLVM_VERSION_CODE >= LLVM_VERSION(4, 0)
                ec = memberNameErr ? std::error_code() : errorToErrorCode(memberNameErr.takeError());
#else
...
#if LLVM_VERSION_CODE >= LLVM_VERSION(3, 9)
                Expected<std::unique_ptr<llvm::object::Binary>> child = childOrErr->getAsBinary();
                if (!child)
                    ec = errorToErrorCode(child.takeError());
#else
                ErrorOr<std::unique_ptr<llvm::object::Binary>> child = childOrErr->getAsBinary();
                ec = child.getError();
#endif
                if (ec) {
                    ...
                    errorMsg = "Object file " + child.get()->getFileName().str() + " in archive is not supported";
                    return false;
                } else {
    ...
    if (magic.is_object()) {
        errorMsg = "Loading file " + fileName + " Object file as input is currently not supported";
        return false;
    }
    ...
    std::unique_ptr<llvm::Module> module(parseIR(Buffer, Err, context));
    if (!module) {
        klee_error("Loading file %s failed: Unrecognized file type.", fileName.c_str());
    }
    modules.push_back(std::move(module));
    return true;
}
