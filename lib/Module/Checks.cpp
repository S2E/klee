...
bool DivCheckPass::runOnModule(Module &M) {
    ...
    auto divZeroCheckFunction = cast<Function>(M.getOrInsertFunction("klee_div_zero_check", Type::getVoidTy(ctx),
    ...
    for (auto &divInst : divInstruction) {
        ...
        auto denominator =
            Builder.CreateIntCast(divInst->getOperand(1), Type::getInt64Ty(ctx), false, /* sign doesn't matter */
                                  "int_cast_to_i64");
...
bool OvershiftCheckPass::runOnModule(Module &M) {
        ...
        for (auto &BB : F) {
            for (auto &I : BB) {
                ...
                if (auto coOp = dyn_cast<llvm::ConstantInt>(operand)) {
                    auto typeWidth = binOp->getOperand(0)->getType()->getScalarSizeInBits();
    ...
    auto overshiftCheckFunction =
        cast<Function>(M.getOrInsertFunction("klee_overshift_check", Type::getVoidTy(ctx), Type::getInt64Ty(ctx),
                                             Type::getInt64Ty(ctx) KLEE_LLVM_GOIF_TERMINATOR));
    ...
    for (auto &shiftInst : shiftInstructions) {
        ...
        auto shiftValue =
            Builder.CreateIntCast(shiftInst->getOperand(1), Type::getInt64Ty(ctx), false, /* sign doesn't matter */
                                  "int_cast_to_i64");
